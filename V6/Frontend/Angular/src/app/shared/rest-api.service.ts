...
export class RestApiService {
  ...
  httpOptions = {
    ...
    getChain(): Observable<Chain> {
      return this.http.get<Chain>(this.serverURLV6 + '/chain')
      .pipe(
        retry(1),
    ...
    postProduction(){
      ...
      return this.http.post(this.serverURLV6 + '/solar_production', this.testProd)
      //TODO
    ...
    postChain(object: transaction): any {
      this.http.post<Chain>(this.serverURLV6 + '/new_transaction', object)
      .toPromise()
      .then(data => {
        console.log(data);
      })
    ...
    connectToChain(value: port): any {
      this.http.post(this.serverURLV6 + '/register_with', value)
      .toPromise()
      .then(data => {
        console.log(data);
      })
    ...
    ///THIS IS BIDDDDDDDD
    postConsumption(){
      return this.http.post(this.serverURLV6 + '/solar_consumption', this.testProd)
    }

    postBid(bid: bid){
      return this.http.post(this.serverURLV6 + '/validate_bid', bid)
    }

    //get the surplus from the server from the weather server
    postSurplus( ){
      
    //testing if data came through okay
    this.testPost.author = this.currentUid;
    if(this.currentUid = "") 
    {
      this.testProd.author = "NOtcKxDvogdK9JbDTacpUbb1EiC2";
    } 

      return this.http.post(this.serverURLV6 + '/surplus',  this.testProd)
    }
